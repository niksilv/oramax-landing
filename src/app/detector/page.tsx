...
function isNumberArray(u: unknown): u is number[] {
  return Array.isArray(u) && u.every((x) => typeof x === "number" && Number.isFinite(x));
}
function findNumberArray(u: unknown, depth = 0): number[] {
  if (depth > 6 || u == null) return [];
  if (isNumberArray(u)) return u;
  if (typeof u === "object") {
    const obj = u as Record<string, unknown>;
    // Προτίμησε κλασικά κλειδιά
    for (const key of ["lightcurve", "flux", "y", "values", "lc"]) {
      if (key in obj) {
        const hit = findNumberArray(obj[key], depth + 1);
        if (hit.length) return hit;
      }
    }
    // fallback: ψάξε παντού
    for (const v of Object.values(obj)) {
      const hit = findNumberArray(v, depth + 1);
      if (hit.length) return hit;
    }
  }
  return [];
}
function looksLikeTIC(s: string) {
  return /^TIC\s*\d{6,}$/.test(s.trim().toUpperCase());
}
...
export default function DetectorPage() {
  ...
  const [target, setTarget] = useState("TIC 268042363");
  ...
  const [loading, setLoading] = useState<null | "predict" | "file" | "health" | "fetch">(null);
  ...
  async function handleTargetChange(val: string) {
    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        setSuggest(items); setShowSuggest(true);
      } catch { setSuggest([]); setShowSuggest(false); }
    }, 220);
  }
  function selectSuggestion(s: SuggestItem) {
    setTarget(s.label); setShowSuggest(false);
  }

  async function fetchLightcurveForTarget(): Promise<number[]> {
    if (!looksLikeTIC(target)) throw new Error("Target must be in the form 'TIC 123456789'.");
    setLoading("fetch");
    try {
      const r = await Api.detect({ target });         // backend πρέπει να επιστρέφει lightcurve
      setJsonOut(r.body);
      const arr = findNumberArray(r.body);
      if (!arr.length) throw new Error("Could not extract lightcurve from /detect response.");
      setLcText(JSON.stringify(arr));                 // γεμίζει το textarea για να το βλέπεις/πειράζεις
      return arr;
    } finally {
      setLoading(null);
    }
  ...
  async function onPredict() {
    setLoading("predict"); setError(null); setProb(null);
    try {
      let arr = parseNumbers(lcText);
      if (!arr.length && looksLikeTIC(target)) {
        // auto-fetch από target αν δεν έχεις νούμερα
        arr = await fetchLightcurveForTarget();
      }
      if (!arr.length) throw new Error("Please provide lightcurve numbers or use a TIC target.");
  ...
  async function onPredictFile() {
    if (!file) { setError("Please choose a file (.csv or .txt)."); return; }
    setLoading("file"); setError(null); setProb(null);
    try {
  ...
  return (
    <div className="container" style={{ padding: "24px 16px", maxWidth: 960, margin: "0 auto" }}>
      <h1 style={{ fontWeight: 800, fontSize: 36, marginBottom: 16 }}>Exoplanet Detector</h1>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 14 }}>{apiOk == null ? "" : apiOk ? "API OK" : "API error"}</span>
      </div>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 8 }}>
          <button onClick={fetchLightcurveForTarget} disabled={loading === "fetch"} className="btn">
            {loading === "fetch" ? "Fetching…" : "Fetch lightcurve from target"}
          </button>
        </div>
      </div>
      ...
      <pre className="output" style={{
        padding: 12, border: "1px solid var(--border, #333)",
        background: "var(--surface-2, #0a0a0a)", borderRadius: 6, overflow: "auto", fontSize: 12
      }}>
        {rendered}
      </pre>

      <style jsx global>{`
        .btn { padding: 6px 10px; border: 1px solid var(--border, #333);
               background: transparent; border-radius: 6px; font-size: 12px; }
        .btn:hover { background: rgba(255,255,255,0.05); }
        .input { padding: 8px 10px; border: 1px solid var(--border, #333);
                 background: transparent; border-radius: 6px; color: inherit; }
      `}</style>
    </div>
  );
}
