...
export default function DetectorPage() {
  ...
  const [jsonOut, setJsonOut] = useState<any>({});
  ...
  async function onSuggest(q: string) {
    ...
    tmr.current = setTimeout(async () => {
      const r = await Api.suggest(q, "TESS");
      const items = Array.isArray(r.body?.items) ? (r.body.items as SuggestItem[]) : [];
      setSuggest(items);
    }, 220);
  }
    ...
    <div className="px-6 py-8 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm flex items-center gap-3">
          <button onClick={checkApi} className="px-2 py-1 rounded-md border text-xs">
            Check API
          </button>
