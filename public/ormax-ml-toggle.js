(() => {
  console.log("[ORAMAX] toggle script loaded (root public)");
  ...
  // 1) Ρητά globals που μπορεί να έχετε ήδη
  if (window.DETECTOR_TIME && window.DETECTOR_FLUX) { return { time: window.DETECTOR_TIME, flux: window.DETECTOR_FLUX }; }
  if (window.time && window.flux && Array.isArray(window.time) && Array.isArray(window.flux)) { return { time: window.time, flux: window.flux }; }

  // 2) Κοινά namespaces / libs
  const hints = [
    ["App","series"], ["App","lc"], ["LC","data"], ["LC","series"],
    ["chart","series","0","data"], ["plot","data"]
  ];
  for (const h of hints) {
    try {
      let ref = window;
      for (const k of h) { ref = ref?.[k]; }
      if (ref && Array.isArray(ref)) {
        const arr = ref;
        // αν είναι array από [x,y]
        if (arr.length>50 && Array.isArray(arr[0]) && arr[0].length>=2) {
          const t = arr.map(p=>Number(p[0])).filter(n=>Number.isFinite(n));
          const f = arr.map(p=>Number(p[1])).filter(n=>Number.isFinite(n));
          if (t.length>50 && t.length===f.length) return { time:t, flux:f };
        }
      }
    } catch {}
  }

  // 3) Γενικός scanner: βρες 2 μεγάλα numeric arrays ~ίδιου μήκους
  try {
    const candidates = [];
    for (const [k,v] of Object.entries(window)) {
      if (Array.isArray(v) && v.length>50 && typeof v[0] !== "object") {
        const nums = v.map(Number).filter(Number.isFinite);
        if (nums.length === v.length) candidates.push([k, nums]);
      }
    }
    candidates.sort((a,b)=>b[1].length-a[1].length);
    for (let i=0;i<candidates.length;i++){
      for (let j=i+1;j<candidates.length;j++){
        const A=candidates[i][1], B=candidates[j][1];
        if (Math.abs(A.length - B.length) <= 5 && Math.min(A.length,B.length) > 200) {
          return { time: A, flux: B };
        }
      }
    }
  } catch {}
  return { time:null, flux:null };
