...
(function(){
  ...
  async function suggestApi(q){
    ...
    return arr.map(it=>{
      ...
      const num = id.replace(/^TIC\s+/i,'').trim();     // για dropdown: σκέτος αριθμός
  ...
  // Αν πατηθεί Fetch & Detect και ο χρήστης έγραψε σκέτο νούμερο, βάλτο ως "TIC <num>" πριν φύγει το request
  ...
  // Τύλιγμα της υπάρχουσας applyDetectResult (αν υπάρχει)
